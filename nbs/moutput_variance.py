...
if __name__ == "__main__":
    ...
    epochs = 2500
    log_freq = 50
    model_names = ["CESM", "CANESM", "CSIRO", "GFDL_esm2m", "ECEARTH"]
    standardisers = {}
    for mn in model_names:
        df = pd.read_csv(f"/home/pindert2/bayesian_ensembling/nbs/data/SingLoc_{mn}.csv")
        ...
        rsubset = realisations[: n_years * 365, :]
        rsubset.shape
        ...
        test_realisations = realisations[(n_years * 365) : (n_years + n_years_predict) * 365, :]
        ...
        yte = realisations[: (n_years + n_years_predict) * 365, :]
        Xte = np.arange(0, (n_years + n_years_predict) * 365).reshape(-1, 1).astype(np.float64)
        Xte = (Xte - Xtr_mu) / Xtr_std
        Xte_idx = np.arange(0, (n_years + n_years_predict) * 365).astype(np.float64)
        ...
        df.to_csv(f"output/preds/model_output_{mn}.csv", index=False)
