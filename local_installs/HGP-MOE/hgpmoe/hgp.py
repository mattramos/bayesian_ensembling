...
class HierarchicalGP(
    gpflow.models.BayesianModel, InternalDataTrainingLossMixin
):
    def __init__(
        self,
        data,
        ...
        for g in self.G:
            if taper:
                self.K_group_list.append(
                    TaperedMatern(
                        kern_type="Matern52", name="group_kernel%d" % int(g)
                    )
                )
            else:
                self.K_group_list.append(
                    gpflow.kernels.Matern52(name="group_kernel%d" % int(g))
                )
        ...
        for i in range(len(self.X)):
            ...
            g_idx = g_idx[0]
            self.K_patient_list.append(
                gpflow.kernels.Matern32(name="patient_kernel%d" % i)
            )
    ...
    def log_marginal_likelihood(self, patient_idx=None):
        ...
        if (patient_idx is not None) and (None not in self.NLL_sum):
            ...
            Yg = tf.concat(Yg, 0)
            linop_blocks = [
                tf.linalg.LinearOperatorFullMatrix(block) for block in blocks
            ]
            linop_block_diagonal = tf.linalg.LinearOperatorBlockDiag(
                linop_blocks
            )
            ...
            for g in self.G:
                ...
                Yg = tf.concat(Yg, 0)
                linop_blocks = [
                    tf.linalg.LinearOperatorFullMatrix(block)
                    for block in blocks
                ]
                linop_block_diagonal = tf.linalg.LinearOperatorBlockDiag(
                    linop_blocks
    ...
    def predict_y(
        self, Xnew, patient_idx, full_cov=False, full_output_cov=False
    ):
        """
    ...
    def predict_f(
        ...
        (g_idx,) = np.where(self.G == g)
        g_idx = g_idx[0]
        kmm = self.K_patient_list[patient_idx](Xi) + self.K_group_list[g_idx](
            Xi
        )
        knn = self.K_patient_list[patient_idx](
            Xnew, full_cov=full_cov
        ) + self.K_group_list[g_idx](Xnew, full_cov=full_cov)
        kmn = self.K_patient_list[patient_idx](Xi, Xnew) + self.K_group_list[
            g_idx
        ](Xi, Xnew)
    ...
    def predict_f_samples(
        ...
        if full_cov:
            ...
            samples = sample_mvn(mean_for_sample, cov, full_cov, num_samples=d)
    ...
    def predict_log_density(
        self, data, patient_idx, full_cov=False, full_output_cov=False
    ):
        """
        ...
        X, Y = data
        f_mean, f_var = self.predict_f(
            X, patient_idx, full_cov=full_cov, full_output_cov=full_output_cov
        )
