...
class Barycentre(EnsembleScheme):
    def __init__(self, name: str = "Barycentre") -> None:
        super().__init__(name)

    @abc.abstractmethod
    def _compute(
        self, process_models: ModelCollection, weights: np.ndarray
    ) -> jnp.DeviceArray:

        mvns = process_models.multivariate_gaussian_set()
        ...
        for t_idx in trange(process_models[0].n_observations):
            ...
            bary_mu, bary_std = gaussian_barycentre(
                np.asarray(means_t),
                np.asarray(stds_t),
                weight)
...
class MultiModelMean(EnsembleScheme):
...
class WeightedModelMean(EnsembleScheme):
    def __init__(self, name: str = "MultiModelMean") -> None:
        super().__init__(name)

    @abc.abstractmethod
    def _compute(
        self, process_models: ModelCollection, weights: jnp.ndarray
        ) -> jnp.DeviceArray:
        assert np.logical_and(weights.ndim == 1, weights.shape[0] == process_models.number_of_models), 'Weights must be 1D and have the same length as the number of models'
        weighted_mean = 0.
        weighted_var = 0.
        ...
        return np.asarray([distrax.Normal(m, s) for m,s in zip(weighted_mean, jnp.power(weighted_var, 0.5))])
